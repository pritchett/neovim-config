...
function M.set_keymaps(client, bufnr)
  ...
  keymap('n', 'gI', '<CMD>Telescope lsp_implementations theme=ivy<CR>zz', with_defaults({desc = "Go To Implementation"}))
  ...
  keymap('n', '<Leader>ls', '<CMD>Telescope lsp_dynamic_workspace_symbols theme=ivy<CR>', with_defaults({desc = "Search Workspace Symbols"}))
  keymap('n', '<Leader>lr', vim.lsp.buf.rename, with_defaults({desc = "Rename Symbol"}))
  -- keymap('n', '<leader><space>', vim.lsp.buf.code_action, opts)
  keymap('n', '<leader>ld', function() vim.diagnostic.open_float({ border = "rounded" }, {focus=false, scope="cursor"}) end, with_defaults({desc = "Show Diagnostics"}))
  keymap('n', '<leader>cs', function() vim.diagnostic.setqflist({open = false, severity = vim.diagnostic.severity.ERROR}); vim.api.nvim_command('botright cwindow') end, with_defaults({desc = "Open Diagnostics"}))
  keymap('n', '<F9>', function() vim.diagnostic.setqflist({severity = vim.diagnostic.severity.ERROR}) end, with_defaults({desc = "Populate quickfix with project wide compilation errors"}))
...
--    }, bufnr)
  ...
  -- print(vim.inspect(vim.lsp.protocol.resolve_capabilities(capabilities)))
end
