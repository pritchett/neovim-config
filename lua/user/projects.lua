...
M.project_start = function(project_name)
  if (vim.g.project_name) then return end
  vim.system({ "kitten", "@set-tab-title", project_name })
  vim.system({ "kitten", "@set-user-vars", "--match", "id:" .. vim.env.KITTY_WINDOW_ID, "NVIM_PROJECT_NAME=" ..
  project_name })
  vim.system({ "kitten", "@set-user-vars", "--match", "id:" .. vim.env.KITTY_WINDOW_ID, "NVIM_SERVER_NAME=" ..
  vim.v.servername })
...
M.project_remote_start_async = function(project_name, switch, cb)
  ...
  local switchfn = function(project)
    ...
    vim.system(switch_cmd, {}, function(out) run_if_cb(project) end)
  end
  ...
  wait_till_running = function(times)
    if (times < 10000) then
      vim.schedule(function() wait_till_running(times + 1) end)
    else
      M.find_project_async(project_name, {
        not_found = function()
          vim.schedule(function() wait_till_running(0) end)
        end,
        found = run_if_cb
      })
    end
  ...
  M.find_project_async(project_name, {
    not_found = function()
      vim.system(cmd, {}, function(_) wait_till_running(0) end)
    end,
    found = switch_and_cb
  })
end
